...
interface LeaderboardTableRealtimeProps {
  sportId: string | null
  initialStandings: any[]
}

export function LeaderboardTableRealtime({ sportId, initialStandings }: LeaderboardTableRealtimeProps) {
  const [standings, setStandings] = useState(initialStandings)
  const supabase = getSupabaseBrowserClient()

  useEffect(() => {
    // Subscribe to changes in leaderboards table
    const channel = supabase
      ...
      .on(
        ...
        async () => {
          console.log("[v0] Real-time leaderboard update")
