...
interface SparkleNavbarProps {
   ...
   * Each object should have a href and label property.
   * @example [{ href: '/', label: 'Home' }, { href: '/about', label: 'About' }]
   */
      ...
      <nav className="navigation-menu" ref={navRef}>
        <ul>
          {items.map((item, index) => (
            <li key={item.href} className={item.href === pathname ? "active" : ""}>
              <Link
                href={item.href}
                ref={(el) => {
                ...
                className="text-foreground px-3.5 py-2 text-sm font-medium rounded-lg transition-all duration-300 hover:text-blue-600"
              >
                {item.label}
              </Link>
            </li>
