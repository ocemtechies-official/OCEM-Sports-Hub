 ...
 * Run with: npx ts-node scripts/populate-sample-data.ts
 */
...
async function populateSampleData() {
  ...
  try {
    ...
    const { data: insertedSports } = await supabase.from('sports').select('id, name')
    ...
    const { data: insertedQuizzes } = await supabase.from('quizzes').select('id, title')
